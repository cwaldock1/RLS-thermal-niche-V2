...
CoralWatch <- as_data_frame(read.csv(file = 'data_derived/RLS_Temperature_CoralWatch0.05.csv'))
CoralWatch$Latitude.1  <- round(CoralWatch$Latitude)
CoralWatch$Longitude.1 <- round(CoralWatch$Longitude)

AllTemps <- left_join(CoralWatch, OldTemperature, by = c('Latitude.1', 'Longitude.1'))

# Load in all RLS data to get survey ID and site code keys. 
RLS  <- as_data_frame(read.csv('data_raw/RLS_GLOBAL_ABUND_BIOMASS_20170507.csv')) # nrow = 587,840
SiteCode_key <- RLS %>% dplyr::select(SiteCode, SurveyID) %>% unique()

# Join temperatures to sites 
SiteTemps_All <- left_join(AllTemps %>% dplyr::select(-Longitude, -Latitude, -X), SiteCode_key)
names(SiteTemps_All) <- c("SurveyID", "MeanTemp_CoralWatch", "MinTemp_CoralWatch", "MaxTemp_CoralWatch", "SDTemp_CoralWatch", "Latitude.1",
                          'Longitude.1', 'RowID', 'MeanSiteSST_NOAA', 'MeanSiteSummerSST_NOAA', 'MeanSiteWinterSST_NOAA', 'SiteCode')

# Aggregate temperatures to site level 
SiteTemps_Agg <- SiteTemps_All %>% group_by(SiteCode) %>% do(MeanTemp_CoralWatch = mean(.$MeanTemp_CoralWatch),
                                        MinTemp_CoralWatch = mean(.$MinTemp_CoralWatch), 
                                        MaxTemp_CoralWatch = mean(.$MaxTemp_CoralWatch), 
                                        SDTemp_CoralWatch = mean(.$SDTemp_CoralWatch), 
                                        MeanSiteSST_NOAA = mean(.$MeanSiteSST_NOAA), 
                                        MeanSiteSummerSST_NOAA = mean(.$MeanSiteSummerSST_NOAA), 
                                        MeanSiteWinterSST_NOAA = mean(.$MeanSiteWinterSST_NOAA, na.rm = T), 
                                        Latitude.1 = mean(.$Latitude.1), 
                                        Longitude.1 = mean(.$Longitude.1)) %>% unnest()
...
pdf('figure_final/review-figures/SST-comparison.pdf', width = 5, height = 5)
ggplot(data = SiteTemps_Agg) + 
  geom_point(aes(y = MeanSiteSummerSST_NOAA, x = MaxTemp_CoralWatch), fill = 'red3', col = 'red3', pch = 21, alpha = 0.5) + 
  stat_smooth(aes(y = MeanSiteSummerSST_NOAA, x = MaxTemp_CoralWatch), col = 'black', method = 'lm', se = T) + 
  
  geom_point(aes(y = MeanSiteWinterSST_NOAA, x = MinTemp_CoralWatch), fill = 'blue3', col = 'blue3', pch = 21, alpha = 0.5) + 
  stat_smooth(aes(y = MeanSiteWinterSST_NOAA, x = MinTemp_CoralWatch), col = 'black', method = 'lm' ) + 
  
  geom_point(aes(y = MeanSiteSST_NOAA, x = MeanTemp_CoralWatch), fill = 'gray70', col = 'gray70', pch = 21, alpha = 0.5) + 
  stat_smooth(aes(y = MeanSiteSST_NOAA, x = MeanTemp_CoralWatch), col = 'black', method = 'lm' ) + 
  
  theme_bw() + 
  theme(panel.grid = element_blank(), aspect.ratio = 0.75) + 
  geom_abline() + 
  xlab('Coral Watch SST product \n (daily 0.05°C aggregated to 2-year prior to survey)') + 
  ylab('NOAA SST product \n (1°C aggregated to 10-year prior to survey)')
dev.off()


# Model and plot of min and max with mean from fine scale resolution data 
lm_MinTempVSMean <- lm(MeanTemp_CoralWatch ~ MinTemp_CoralWatch, data = SiteTemps_Agg)
lm_MaxTempVSMean <- lm(MeanTemp_CoralWatch ~ MaxTemp_CoralWatch, data = SiteTemps_Agg)
summary(lm_MinTempVSMean)
summary(lm_MaxTempVSMean)

pdf('figure_final/review-figures/SST-CoralWatch-comparison.pdf', width = 5, height = 5)
ggplot(data = SiteTemps_Agg) + 
  geom_point(aes(y = MeanTemp_CoralWatch, x = MaxTemp_CoralWatch), fill = 'red3', col = 'red3', pch = 21, alpha = 0.5) + 
  stat_smooth(aes(y = MeanTemp_CoralWatch, x = MaxTemp_CoralWatch), col = 'black', method = 'lm', se = T) + 
  
  geom_point(aes(y = MeanTemp_CoralWatch, x = MinTemp_CoralWatch), fill = 'blue3', col = 'blue3', pch = 21, alpha = 0.5) + 
  stat_smooth(aes(y = MeanTemp_CoralWatch, x = MinTemp_CoralWatch), col = 'black', method = 'lm' ) + 
  theme_bw() + 
  theme(panel.grid = element_blank(), aspect.ratio = 0.75) + 
  geom_abline() + 
  ylab('Coral Watch SST product mean values \n (daily 0.05°C aggregated to 2-year prior to survey)') + 
  xlab('Coral Watch SST product extremes \n (daily 0.05°C aggregated to 2-year prior to survey)')
dev.off()

# The offset part of maximum temperature is most likly the enclosed part of the med around spain c.f. portugal which is cooler. 
pdf('figure_final/review-figures/SST-CoralWatch-comparison.pdf', width = 5, height = 5)
ggplot(data = SiteTemps_Agg) + 
  geom_point(aes(x = Latitude.1, y = MaxTemp_CoralWatch), fill = 'red3', col = 'red3', pch = 21, alpha = 0.5) + 
  #stat_smooth(aes(x = Latitude.1, y = MaxTemp_CoralWatch), col = 'black', method = 'lm', se = T) + 
  
  geom_point(aes(x = Latitude.1, y = MinTemp_CoralWatch), fill = 'blue3', col = 'blue3', pch = 21, alpha = 0.5) + 
  #stat_smooth(aes(x = Latitude.1, y = MinTemp_CoralWatch), col = 'black', method = 'lm' ) + 
  theme_bw() + 
  theme(panel.grid = element_blank(), aspect.ratio = 0.75) 
dev.off()


### Link in MPA criteria wrt review threes comments for abundance models ----

MPA_status <- read.csv('data_raw/NEOLI_update 08-18.csv')

view(dfSummary(MPA_status))

head(MPA_status)

MPA_status$Governance..no.take.multizone..no.take.separate..general.use.multizone..restricted.take..open...

# Sort out column names 
MPA_status <- plyr::rename(MPA_status, c('Governance..no.take.multizone..no.take.separate..general.use.multizone..restricted.take..open...' = 'Governance', 
                           'year.of.declaration' = 'YearDesignated', 
                           'area..km2.' = 'Sizekm2', 
                           'offshore.extent..km.' = 'AreaOffshorekm', 
                           'Distance.to.boundary..km.' = 'DistanceToEdge'))

# Filter to relevant columns 
MPA_status <- MPA_status %>% dplyr::select(SiteCode, MPA, MPA.1, Zone.name, Isolation, Effectiveness, Governance, YearDesignated, Sizekm2)

# Relevel factors to represent 1 and 0 for NEOLI scoring. 
MPA_status$Effectiveness <- revalue(MPA_status$Effectiveness, c(h = "1", H = "1", High = "1", l = '0', L = '0', m = '0', M = '0',' ' = '0', '#N/A' = '0'))
MPA_status$Effectiveness[is.na(MPA_status$Effectiveness)] <- '0'

MPA_status$Governance <- revalue(MPA_status$Governance, c(NT = "1", NTZ = "1", O = "0", R = '0', RZ = '0'))

quantile(MPA_status$YearDesignated[MPA_status$YearDesignated > 0], na.rm = T, 0.66) # Choose tirciles. 
MPA_status$YearDesignated[is.na(MPA_status$YearDesignated)] <- 0
MPA_status$YearDesignated[MPA_status$YearDesignated > 0]<- ifelse(MPA_status$YearDesignated[MPA_status$YearDesignated > 0] < 2004, 1, 0)

MPA_status$Sizekm2[is.na(MPA_status$Sizekm2)] <- 0
MPA_status$Sizekm2 <- ifelse(MPA_status$Sizekm2 > 100, 1, 0)

MPA_status$Effectiveness   <- as.numeric(as.character(MPA_status$Effectiveness))
MPA_status$Isolation       <- as.numeric(as.character(MPA_status$Isolation))
MPA_status$Governance      <- as.numeric(as.character(MPA_status$Governance))
MPA_status$YearDesignated  <- as.numeric(as.character(MPA_status$YearDesignated))
MPA_status$Sizekm2         <- as.numeric(as.character(MPA_status$Sizekm2))

MPA_status$NEOLI <- MPA_status$Effectiveness + MPA_status$Isolation + MPA_status$Governance + MPA_status$YearDesignated + MPA_status$Sizekm2
MPA_status$NEOLI[is.na(MPA_status$NEOLI)] <- 0
hist(MPA_status$NEOLI)

# Select just column of interest
MPA_status <- MPA_status %>% select(SiteCode, NEOLI)
write.csv(MPA_status, 'data_derived/MPA_Status_Edit_25-09-2018.csv', row.names = F)

# What percentages for NEOLI
round(table(MPA_status$NEOLI) / length(unique(MPA_status$SiteCode)) * 100 , 2)

RLS_19 <- left_join(RLS_19, MPA_status)
